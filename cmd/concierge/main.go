...
db,err:=store.Open(dataDir);if err!=nil{log.Fatalf("concierge: %v",err)};defer db.Close();srv:=server.New(db,server.DefaultLimits())
fmt.Printf("\n  Concierge — Self-hosted customer onboarding checklists\n  Dashboard:  http://localhost:%s/ui\n  API:        http://localhost:%s/api\n\n",port,port)

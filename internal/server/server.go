package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-concierge/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-concierge"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/checklists",s.handleList)
    s.mux.HandleFunc("POST /api/checklists",s.handleCreate)
    s.mux.HandleFunc("DELETE /api/checklists/{id}",s.handleDelete)
    s.mux.HandleFunc("GET /api/checklists/{id}/progress",s.handleGetProgress)
    s.mux.HandleFunc("POST /api/checklists/{id}/progress",s.handleUpdateProgress)

...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{
			"tier":        s.limits.Tier,
			"upgrade_url": "https://stockyard.dev/concierge/",
		})
	})
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("concierge: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("concierge: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"checklists": oe(s.db.Search(q, filters))})
		return
	}
...
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	if s.limits.MaxItems > 0 && s.db.Count() >= s.limits.MaxItems {
		we(w, 402, "Free tier limit reached. Upgrade at https://stockyard.dev/concierge/")
		return
	}
	var e store.Checklist
	if err := json.NewDecoder(r.Body).Decode(&e); err != nil {
		we(w, 400, "invalid json")
		return
	}
	if e.CustomerName == "" {
		we(w, 400, "customer_name required")
		return
	}
...
func (s *Server) update(w http.ResponseWriter, r *http.Request) {
	...
	if patch.Template == "" {
		patch.Template = existing.Template
	}
	if patch.Steps == "" {
		patch.Steps = existing.Steps
	}
	if patch.Assignee == "" {
		patch.Assignee = existing.Assignee
	}
	if patch.Status == "" {
